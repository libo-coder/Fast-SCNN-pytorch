...
def parse_args():
    ...
    parser.add_argument('--weight-decay', type=float, default=1e-4, metavar='M', help='w-decay')
    # checking point
    parser.add_argument('--resume', type=str, default=None, help='put the path to resuming file if needed')
    parser.add_argument('--save-folder', default='./weights', help='Directory for saving checkpoint models')
    # evaluation only
...
class Trainer(object):
    ...
    def validation(self, epoch):
        ...
        for i, (image, target) in enumerate(self.val_loader):
            ...
            print('Epoch %d, Sample %d, validation pixAcc: %.3f%%, mIoU: %.3f%%' % (epoch, i + 1, pixAcc * 100, mIoU * 100))
