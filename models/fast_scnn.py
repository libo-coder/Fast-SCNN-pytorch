...
class FastSCNN(nn.Module):
    ...
    def forward(self, x):
        size = x.size()[2:]
        print('size:', size)
        print('x:', x.shape)        # [1, 3, 1024, 2048]
        higher_res_features = self.learning_to_downsample(x)
        print('higher_res_features.shape:', higher_res_features.shape)      # [1, 64, 128, 256]
        x = self.global_feature_extractor(higher_res_features)
        print('x2:', x.shape)       # [1, 128, 32, 64]
        x = self.feature_fusion(higher_res_features, x)
        print('x3:', x.shape)       # [1, 128, 128, 256]
        x = self.classifier(x)
        print('x4:', x.shape)       # [1, 19, 128, 256]
        outputs = []
        x = F.interpolate(x, size, mode='bilinear', align_corners=True)     # 插值和上采样
        print('x5:', x.shape)       # [1, 19, 1024, 2048]
        outputs.append(x)
...
def get_fast_scnn(dataset='citys', pretrained=False, root='./weights', map_cpu=False, **kwargs):
    ...
    print('datasets[dataset].NUM_CLASS:', datasets[dataset].NUM_CLASS)
